...
router.post("/signup", async (req, res) => {
  try {
    ...
    const user = new User({
      ...
      bg_clr: random_clr,
      cover_pic:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSuGtIXUGOHsmxJL3mQRqFe1K9xclHAJzAQOQ&s",
...
router.get("/home", async (req, res) => {
  try {
    ...
    console.error("error in auth /home:", error.message);
    res.json([]);
  }
});
...
router.get("/all_post_comments", async (req, res) => {
  const all_sentence = await Sentence.find()
    .populate({
      path: "comments",
      populate: { path: "userId", model: "User" },
    })
...
router.put("/set_comment_this_post", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const populatedPost = await Sentence.findById(id).populate({
      path: "comments",
      populate: { path: "userId", model: "User" },
    });
